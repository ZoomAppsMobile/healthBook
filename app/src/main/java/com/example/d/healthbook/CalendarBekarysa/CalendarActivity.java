...
import com.example.d.healthbook.DialogFragments.DialogFragmentUpdateDrugs;
...
//    private TextView toolbar_title;
   ...
   private int all_period_woman=0;
    ...
    //    Toast.makeText(this, SharedPreference.getDataBegin(this), Toast.LENGTH_SHORT).show();
...
//WOMAN
          ...
          if(!SharedPreference.getDataBegin(CalendarActivity.this).equals("")){

              if(compareTwoDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate())){



                 if(getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate()) < Integer.valueOf(SharedPreference.getWomanduration(CalendarActivity.this))

                     & getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate()) >=Integer.valueOf(SharedPreference.getWomanPeriod(CalendarActivity.this))

                         ||
                         getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate()) < Integer.valueOf(SharedPreference.getWomanduration(CalendarActivity.this))*2

                                 & getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate()) >=Integer.valueOf(SharedPreference.getWomanPeriod(CalendarActivity.this))+Integer.valueOf(SharedPreference.getWomanduration(CalendarActivity.this))
                     ...
                     if(getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate())>7 & getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate())<18){
                     ...
                     if(getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate())==13){
                         calendarDatum.get(i).setEvolition(true);
                         calendarDatum.get(i).setWomanDay(false);

                     }

                }
               else if(getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate()) < Integer.valueOf(SharedPreference.getWomanPeriod(CalendarActivity.this)) ||
                  getDaysBetweenDates(SharedPreference.getDataBegin(CalendarActivity.this),calendarDatum.get(i).getDate()) < Integer.valueOf(SharedPreference.getWomanPeriod(CalendarActivity.this)) +Integer.valueOf(SharedPreference.getWomanduration(CalendarActivity.this))
                      ){
...
//WOMAN END
            ...
            if (calendarDatum.get(i).getDate().equals(splitdate[0])) {
                ...
                if (responseNoteType1.getType() == 1) {
                    ...
                    default_drugs_period=i+responseNoteType1.getDuration();
                    all_period=responseNoteType1.getDuration();
                }
                    ...
                    SharedPreference.setDataBegin(CalendarActivity.this,data_begin);
                    SharedPreference.setWomanduration(CalendarActivity.this, String.valueOf(responseNoteType1.getDuration()));
                    SharedPreference.setWomanPeriod(CalendarActivity.this, String.valueOf(responseNoteType1.getDurationPeriod()));
    ...
    public void onDateSelected(final CalendarModel out) {
        runOnUiThread(new Runnable() {
          ...
          //       Toast.makeText(CalendarActivity.this, out.getDDate(), Toast.LENGTH_SHORT).show();
                dateStr = out.getDate();
    ...
    void showWaitToast(){
        Toast.makeText(CalendarActivity.this,"Подождите, идет загрузка...",Toast.LENGTH_SHORT).show();
    }
    @Override
    protected void onCreate(Bundle savedInstanceState) {
       ...
       add_note_fab = (FloatingActionButton) findViewById(R.id.add_note_fab);
        ...
        navigationClickItemListener(CalendarActivity.this , CalendarActivity.class);
    ...
    private void ClickListenerBtnToolbar() {
        ...
        womenCalendarBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(isDataLoaded){
                    Bundle bundle = new Bundle();
                    bundle.putString("date", "sfsd");
                  bundle.putSerializable("lstContact", (Serializable) objectList);


                dialogFragmentWomanCalendar = new WomanCalendarFragment();
                dialogFragmentWomanCalendar.setArguments(bundle);
    ...
    public  void updateDrugs (Event_Data data){
        ...
        if(isDataLoaded){
            ...
            DialogFragmentUpdateDrugs dialogFragmentAddDrugs = new DialogFragmentUpdateDrugs();
            dialogFragmentAddDrugs.setArguments(bundle);
            dialogFragmentAddDrugs.show(getSupportFragmentManager(), "dialogFragmentAddDrugs");
        }
        else{
    ...
    public void seeNote1(String type) {
        ...
        App.getApi().seeNoteType1(GlobalVariables.user_auth_token).enqueue(new Callback<List<ResponseNoteType1>>() {
            ...
            public void onResponse(Call<List<ResponseNoteType1>> call, Response<List<ResponseNoteType1>> response) {
                ...
                if (response.code() == 200) {
                    ...
                    for (int i = 0; i < response.body().size(); i++) {
                        ...
                        if(responseCalBody.get(i).getType()==3){

                            String splitdate_data_begin []= responseCalBody.get(i).getDateBegin().split(" ");
                            data_begin=splitdate_data_begin[0];

                            //Toast.makeText(CalendarActivity.this, data_begin, Toast.LENGTH_SHORT).show();

                            SharedPreference.setDataBegin(CalendarActivity.this,data_begin);
                            WomanCycle womanCycle = new WomanCycle();
                            womanCycle.setDataBegin(data_begin);
                            womanCycle.setId(responseCalBody.get(i).getId());
                            objectList.add(womanCycle);
                            SharedPreference.setWomanduration(CalendarActivity.this, String.valueOf(responseCalBody.get(i).getDuration()));
                            SharedPreference.setWomanPeriod(CalendarActivity.this, String.valueOf(responseCalBody.get(i).getDurationPeriod()));
                        }
                        String date = "";
            ...
            public void onFailure(Call<List<ResponseNoteType1>> call, Throwable t) {
                Toast.makeText(CalendarActivity.this, "Error SeeNote", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void processEventAll(List<Event_Data> datum, ResponseNoteType1 data) {
        ...
        switch (data.getType()) {
            ...
            case 2: {
                ...
                String[] s = data.getDateBegin().split(" ");

                for (int i=0;i<data.getDuration();i++){

                    Event_Data event_data = new  Event_Data(data.getType(),
                            data.getTitle(),
                            data.getBody(),getDaysByNumber(s[0],i), Time,
                            data.getBody(),data.getId(),data.getDuration());
                    datum.add(event_data);
                    Log.d("dtStr",getDaysByNumber(s[0],i));
    ...
    public String   getDaysByNumber(String dt,int duration){
       ...
       // String dt = "2012-01-04";  // Start date
        ...
        c.add(Calendar.DATE, duration);  // number of days to add, can also use Calendar.DAY_OF_MONTH in place of Calendar.DATE
    ...
    public void updateNewTakingDrugs(String type, String title, String text, String date, String quantityDrugs,
                                   Integer period, Integer duration, List<String> times,String item_id) {
        App.getApi().updateDrug(GlobalVariables.user_auth_token,
                API_Controller.saveNewTakingDrus(type, title, text, date, times, period, duration, quantityDrugs),item_id).enqueue(new Callback<ResponseNoteType1>() {
            @Override
            public void onResponse(Call<ResponseNoteType1> call, Response<ResponseNoteType1> response) {



                 Toast.makeText(CalendarActivity.this, "Изменения сохранены", Toast.LENGTH_SHORT).show();
